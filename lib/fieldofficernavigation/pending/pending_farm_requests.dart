...
import 'package:apps/fieldofficernavigation/pending/detail_pending_farm.dart';
import 'package:apps/model/pendingListModel.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:shared_preferences/shared_preferences.dart';

class PendingFarmRequests extends StatefulWidget {
  @override
  _PendingFarmRequestsState createState() => _PendingFarmRequestsState();
}

class _PendingFarmRequestsState extends State<PendingFarmRequests> {
  ...
  Future<List<PendingListModel>> _getPendingFarmer() async {
    var response = await http.post(
        ...
        body: {"field_officer_id": userId, "type": "farm", "status": "1"});
      ...
      child: InkWell(
          onTap: () {
            Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        DetailPendingFarm(farmInfo: farmList)));
          },
          child: ListTile(
            ...
            trailing: IconButton(
                icon: Icon(Icons.arrow_right, color: Colors.orange[300]),
                onPressed: () {
                  Navigator.of(context).push(MaterialPageRoute(
                      builder: (ctx) => DetailPendingFarm(farmInfo: farmList)));
                }),
          )),
    );
  }
